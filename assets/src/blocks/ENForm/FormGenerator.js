...
import { inputName, inputId } from './inputName';

const { __ } = wp.i18n;

export const FormGenerator = ({fields, attributes, onInputChange, errors}) => {
      ...
      {fields.map((field, index) => {
        ...
        return (
          <Input
            ...
            {...{field, index, onInputChange, dependent_field, errors, is_side_style}}
          />
        );
      })}
    </div>
...
const Input = (props) => {
  ...
  return ((field) => {
    switch (field.input_type) {
      case 'text':
      case 'email':
        return <TextInput {...{field, onInputChange, errors, is_side_style}} />;
      case 'checkbox':
        return <CheckboxInput {...{field, onInputChange, index, dependent_field, errors, is_side_style}} />;
      case 'radio':
        return <RadioInput {...{field, onInputChange, errors, is_side_style}} />;
      case 'country':
        return <CountryInput {...{field, onInputChange, errors, is_side_style}} />;
      case 'position':
        return <PositionInput {...{field, onInputChange, errors, is_side_style}} />;
      case 'hidden':
...
const TextInput = ({field, onInputChange, errors, is_side_style}) => {
...
const CheckboxInput = ({field, onInputChange, index, dependent_field, errors}) => {
  return field.en_type === 'GEN'
    ? <CheckboxGen {...{field, onInputChange, index, dependent_field, errors}} />
    : <CheckboxOpt {...{field, onInputChange, dependent_field, errors}} />
}

const CheckboxOpt = ({field, onInputChange, dependent_field, errors}) => {
  const {id, name} = inputId(field);
...
const CheckboxGen = ({field, onInputChange, index, dependent_field, errors}) => {
  const {id, name} = inputId(field);
  const locale = field.selected_locale;
...
const RadioInput = ({field, onInputChange, errors}) => {
  const {id, name} = inputId(field);
...
const CountryInput = ({field, onInputChange, errors, is_side_style}) => {
  const {id, name} = inputId(field);
  const has_error = errors && errors.includes(field.id);
...
const PositionInput = ({field, onInputChange, errors, is_side_style}) => {
  const {id, name} = inputId(field);
  const has_error = errors && errors.includes(field.id);
